<template>
    <v-card outlined>
        <v-card-title>
            Class # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <String label="ClassName" v-model="item.className" :editMode="editMode" @change="change" />
            </div>
            <div>
                <String label="GosuName" v-model="item.gosuName" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/classes/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
